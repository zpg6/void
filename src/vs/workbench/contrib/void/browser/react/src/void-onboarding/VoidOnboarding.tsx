...
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump, AzureAiFoundrySetupInstructions } from '../void-settings-tsx/Settings.js';
...
const cloudProviders: ProviderName[] = ['googleVertex', 'liteLLM', 'microsoftAzure', 'azureAiFoundry', 'openAICompatible'];
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					...
					{providerName === 'azureAiFoundry' && <AzureAiFoundrySetupInstructions />}
				</div>
			))}
