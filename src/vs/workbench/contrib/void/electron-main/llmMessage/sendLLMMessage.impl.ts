...
const _sendAzureAiFoundryChat = async ({
	messages,
	...
	const formattedMessages = separateSystemMessage
		? [{ role: "system", content: separateSystemMessage }, ...messages.map(m => ({ role: m.role, content: extractContent(m) }))]
	...
	try {
		...
		if (!response.body) {
			throw new Error("No response body was received from Azure AI Inference")
		}
		...
		for await (const event of sseStream) {
			...
			try {
				...
				for (const choice of data.choices || []) {
					...
					if (nameOfReasoningFieldInDelta) {
						// @ts-ignore
						newReasoning = (choice.delta?.[nameOfReasoningFieldInDelta] || '') + ''
						fullReasoningSoFar += newReasoning
					}
		...
		if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
			onError({ message: 'Azure AI Inference: Response was empty.', fullError: null })
		} else {
			const toolCall = rawToolCallObjOf(toolName, toolParamsStr, toolId)
			const toolCallObj = toolCall ? { toolCall } : {}
			onFinalMessage({ fullText: fullTextSoFar, fullReasoning: fullReasoningSoFar, anthropicReasoning: null, ...toolCallObj })
		...
		if (error instanceof Error && error.message?.includes('401')) {
			...
			onError({ message: `Azure AI Inference error: ${error?.message || String(error)}`, fullError: error })
		}
	}
}
...
export const sendLLMMessageToProviderImplementation = {
	...
	azureAiFoundry: {
		sendChat: (params) => _sendAzureAiFoundryChat(params),
		sendFIM: null,
		list: null,
	},
